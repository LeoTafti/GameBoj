...
public final class Bits {
     ...
     * @param size number of bits rotated
     * @param bits original bit-string
    ...
    static public int rotate(int size, int bits, int distance) {
        ...
        int originalClipped = bits - clipped; // 0 in size least-significant bits
        
        int rotated =  clipped << Math.floorMod(distance, size) | //left shift of distance mod(size) bitwise or
                clipped >>> size - Math.floorMod(distance, size); //right shift of size-distance
        
        return  originalClipped + clip(size, rotated); //original 32-size most significant bits and the size rotated least significant bit
    }
